...
// ShareButtons — just the channel chips (WhatsApp / X / email), no section
// chrome. Reused by ShareRow (society pages) and the centered share block on
// /submit/success.
export const ShareButtons: FC<{ url: string; text: string; subject?: string }> = ({ url, text, subject }) => (
...
export const ShareRow: FC<{ url: string; text: string; subject?: string; label?: string }> = ({
  ...
  <section class="px-5 sm:px-8 py-10 sm:py-12 border-t border-hairline">
    <div class="max-w-wide mx-auto flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
      ...
      <ShareButtons url={url} text={text} subject={subject} />
    </div>
  </section>
);
